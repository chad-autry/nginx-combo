...
[Service]
ExecStart=/bin/sh -c "while true; do etcdctl set /{{parent}}/{{group}}/hosts/%H/{{port}}/host '%H' --ttl 60; \
                      {% for item in properties  %}
                      etcdctl set /{{item.key}}/{{item.value.subpath}}/hosts/%H/{{port}}/{{item.key}} '{{item.value}}' --ttl 60; \
                      {% endfor %}
                      sleep 45; \
                      done"
ExecStartPost=-/bin/sh -c '/usr/bin/etcdctl set /{{parent}}/watched "$(date +%s%N)"'
ExecStop=/usr/bin/etcdctl rm /{{parent}}/{{group}}/hosts/%H/{{port}}
